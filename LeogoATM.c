    ...
    while (fscanf(old,"\n %s %s %s \n %lf \n %s \n %s \n %s \n %s\n %d/%d/%d\n %s \n%f",add.name.first_name,add.name.middle_name,add.name.last_name,&add.mobilenumber, add.password, add.emailaddress, add.address, add.nationality,&add.mdy.month,&add.mdy.day,&add.mdy.year,add.placeofbirth,&add.amount)!=EOF)
{
            ...
            { test=1;
                ...
                if(choice==1)
                {
                    ...
                    {
                        fordelay(10000000);
                        gotoxy(7+i,12);printf("\xDB");
                    }
                    gotoxy(7,15);printf("DEPOSITED");
                    gotoxy(7,16);printf("SUCCESSFULLY");

                    gotoxy(7,18);printf("\xAE BALANCE:");
                    gotoxy(10,19);printf("PHP %.2f",add.amount);
   ...
   {
       gotoxy(7,21);printf(" [1] MAIN MENU");
       gotoxy(7,22);printf(" [2] EXIT");
       gotoxy(7,23);printf(" ENTER: ");scanf("%d",&main_exit);
        system("cls");
        if (main_exit==1)
            menu();
        else
